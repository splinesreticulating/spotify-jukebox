import { fetchSongsByArtist, fetchArtistInfo } from '@/app/lib/data'
...
export default async function ArtistPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  const imageUrl = songs.find((song) => Array.isArray(song.image_urls) && song.image_urls.length > 0)?.image_urls?.[1]
    ...
    <main className="mx-auto max-w-[2000px] p-6">
      ...
      <div className="mb-8 flex flex-col gap-6 md:flex-row">
        ...
        <div className="flex flex-col gap-4">
          {artistInfo?.bio && <p className="text-gray-600">{artistInfo.bio}</p>}
          {artistInfo?.tags && artistInfo.tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {artistInfo.tags
                .filter((tag) => !EXCLUDED_TAGS.includes(tag.name.toLowerCase()))
                .map((tag) => (
      ...
      <section>
        <h2 className="mb-4 text-xl font-semibold">{songs.length === 1 ? '1 Nut' : `${songs.length} Nuts`}</h2>
