...
import { ArtistSongView } from '@/app/lib/types/artists'
import { isPlayable } from '@/app/lib/utils'

type SortField = 'title' | 'date_added' | 'album'
type SortDirection = 'asc' | 'desc'

export default function SongTable({ songs: initialSongs }: { songs: ArtistSongView[] }) {
  ...
  const handleSort = (field: SortField) => {
    ...
    const sortedSongs = [...songs].sort((a, b) => {
      if (field === 'title') {
        return (a.title || '').localeCompare(b.title || '') * (newDirection === 'asc' ? 1 : -1)
      }
      if (field === 'date_added') {
        return (a.date_added.getTime() - b.date_added.getTime()) * (newDirection === 'asc' ? 1 : -1)
      }
      if (field === 'album') {
        return (a.album || '').localeCompare(b.album || '') * (newDirection === 'asc' ? 1 : -1)
      }
      return 0
    })
      ...
      <table className="min-w-full table-fixed divide-y divide-gray-200">
        <thead>
          <tr className="bg-gray-50">
            ...
            >
              Title {sortField === 'title' && <SortArrow direction={sortDirection} />}
            </th>
            <th
              scope="col"
            ...
            >
              Album {sortField === 'album' && <SortArrow direction={sortDirection} />}
            </th>
            <th
              scope="col"
            ...
            >
              Added {sortField === 'date_added' && <SortArrow direction={sortDirection} />}
            </th>
          </tr>
        </thead>
          ...
          {songs.map((song) => (
            <tr key={song.id} className="hover:bg-gray-50">
              <td className="w-2/3 px-2 py-3 sm:w-2/5 sm:px-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="max-w-[calc(60vw-3rem)] overflow-hidden sm:max-w-[calc(40vw-6rem)]">
                    <SongLink song={song} showArtist={false} className="block truncate" />
                  </div>
                </div>
              </td>
              ...
              <td className="w-1/3 whitespace-nowrap px-2 py-3 text-sm text-gray-600 sm:w-1/4 sm:px-3">
                {format(new Date(song.date_added), 'MMM d, yyyy')}
              </td>
            </tr>
          ))}
        </tbody>
