...
export default function StatsPage() {
    ...
    >([])
    const [availableSongsByPeriod, setAvailableSongsByPeriod] = useState<
        Array<{ period: string; count: number }>
    >([])
    const [availableSongsByTag, setAvailableSongsByTag] = useState<
        Array<{ tag: string; count: number }>
    >([])
    const [availableSongsByArtist, setAvailableSongsByArtist] = useState<
    ...
    const [showOnlyAvailableArtists, setShowOnlyAvailableArtists] =
        useState(true)

    useEffect(() => {
        const loadData = async () => {
            const [yearData, songsData, periodData, tagData, artistData] =
                await Promise.all([
        ...
        selectedDays,
        showOnlyAvailable,
        showOnlyAvailablePeriod,
        showOnlyAvailableTags,
        showOnlyAvailableArtists,
    ])
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <label className="flex items-center space-x-2 text-sm">
                            <input
                                ...
                                onChange={(e) =>
                                    setShowOnlyAvailablePeriod(e.target.checked)
                                }
                                className="rounded border-gray-300"
                            />
                            <span>Available</span>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <label className="flex items-center space-x-2 text-sm">
                            <input
                                ...
                                onChange={(e) =>
                                    setShowOnlyAvailableTags(e.target.checked)
                                }
                                className="rounded border-gray-300"
                            />
                            <span>Available</span>
