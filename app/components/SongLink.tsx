...
export const SongLink: React.FC<{
  ...
  const getFontClass = (roboticness?: number) => {
    ...
    switch (roboticness) {
      ...
      case 3:
        return isBold ? `${spaceMono.className} font-bold tracking-tighter` : `${spaceMono.className} tracking-tighter`
      default:
        return ''
    }
  }
  ...
  return (
    <span className={clsx('break-words text-sm lowercase sm:text-base', className, fontClass)}>
      <Link href={`/dashboard/songs/${song.id}/edit`} className={`hover:underline ${levelColor}`}>
        {song.title || ''}
      </Link>
        ...
        <span className={levelColor}>
          ...
          {song.artists.map((artist, index) => (
            <React.Fragment key={artist}>
              {index > 0 && ', '}
              <Link href={`/dashboard/artists/${encodeURIComponent(artist)}`} className="hover:underline">
                {artist}
              </Link>
            </React.Fragment>
          ))}
