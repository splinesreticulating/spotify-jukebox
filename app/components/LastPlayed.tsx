...
export const LastPlayed: React.FC<{ song: Song }> = ({ song }) => {
  ...
  const effectiveDate = lastPlayedDate || (song.date_played ? new Date(song.date_played) : null)
  if (!effectiveDate) return null // TypeScript safety

  const days = Number(daysAgo(effectiveDate))
  const beforeThatDays = beforeThatDate ? Number(daysAgo(beforeThatDate)) : null
