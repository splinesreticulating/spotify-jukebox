            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {songs.map((song) => (
                <tr key={song.id} className="transition-colors hover:bg-gray-50">
                  <td className="whitespace-nowrap px-3 py-4">{isPlayable(song) && <PlayButton songId={song.id} />}</td>
                  <td className="whitespace-nowrap py-4 pl-3 pr-4">
                    <SongLink song={{ ...song, id: song.id, level: song.level, roboticness: song.roboticness ?? 2 }} />
                  </td>
