...
export async function GET() {
  try {
    ...
    const currentTime =
      nowLA.getUTCHours().toString().padStart(2, '0') + ':' + nowLA.getUTCMinutes().toString().padStart(2, '0')
    ...
    const currentShow =
      shows.find((show) =>
        (show.rules as any[]).some(
          (rule) =>
            rule.days?.includes(currentDayNumber) && rule.start_time <= currentTime && rule.end_time > currentTime,
        ),
